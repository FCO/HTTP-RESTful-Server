...
has content_type_order => (
	is      => "rw",
	isa     => "ArrayRef",
	default => sub { [qw|application/yaml application/json application/xml text/yaml text/json text/xml|] }
);
...
sub chose {
	...
	if ( defined $req->uri->query_form
		and exists { $req->uri->query_form }->{"content-type"} )
	{
...
sub find_encoders {
		...
		each %{*HTTP::RESTful::Server::ContentType::Encoder::} )
	{
		...
		next
		  if not $obj
			  or not $obj->can("content_type")
			  or exists $self->{encoders}->{ $obj->content_type };
		$self->{encoders}->{ $_ } = $obj->new for @{ $obj->content_type };
	}
	use Data::Dumper;
	print Dumper $self;
}
...
sub encode {
	...
	my $encoded = $self->encoders->{$content_type}->encode($data)
	  if keys %{ $self->encoders } and exists $self->encoders->{$content_type};
}
...
sub decode {
	...
	my $decoded = $self->encoders->{$content_type}->decode($data)
	  if exists $self->encoders->{$content_type};
}
